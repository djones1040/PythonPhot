...
CALLING SEQUENCE:
     from PythonPhot import pkfit_norecent as pkfit
     pk = pkfit.pkfit_class(f, gauss, psf,
                            ronois, phpadu )
     errmag,chi,sharp,niter,scale,xnew,ynew = pk.pkfit_norecent(scale,x,y,sky,radius)
...
EXAMPLE:
     import pyfits
     from PyIDLPhot import pkfit_norecent as pkfit
...
#        psf1d = psf.reshape(shape(psf)[0]**2.) 
        ...
        #        rhosqy,rhosqx = shape(rhosq)[0],shape(rhosq)[1]
            ...
            for kk in range(3):
                ...
                for ll in range(3): c[ll,kk] = sum(t[kk,:][lilrho]*t[ll,:][lilrho]*wt)
