...
def rdpsfmodel(psfmodelfile):
    ...
    if isinstance(psfmodelfile, str):
        ...
        assert len(psfmodelfile) == 3
        gaussparam, lookuptable, psfmag, psfzpt = psfmodelfile
    else:
        raise exceptions.RuntimeError("psfmodel must either be a filename or "
                                      "a tuple with gaussian parameters and a "
                                      "lookup table of non-gaussian "
                                      "components")
...
def get_flux_and_err(imagedat, psfmodel, xy, ntestpositions=100, psfradpix=3,
    ...
    if recenter_target:
        ...
        if x < 0 or y < 0:
            print("WARNING [dophotometry.py] : recentering failed")
            import pdb
            pdb.set_trace()
