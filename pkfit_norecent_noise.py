...
CALLING SEQUENCE:
     from PyPhot import pkfit_norecent_noise as pkfit
     pk = pkfit.pkfit_class(f, gauss, psf,
                            ronois, phpadu )
     errmag,chi,sharp,niter,scale,xnew,ynew = pk.pkfit_norecent_noise(scale,x,y,sky,radius)
...
#        psf1d = psf.reshape(shape(psf)[0]**2.)
        ...
        # D. Jones - modified from Scolnic
            ...
            # D. Jones - noise addition from Scolnic, removed by Scolnic
            ...
            #        rhosqshape = shape(rhosq)
            #        rhosq = rhosq.reshape(rhosqshape[0]*rhosqshape[1])
            ...
            if niter >= 2:    #Reject any pixel with 10 sigma residual
                ...
                nbad = len(badpix)
                # scolnic added - and redundant here
                # sbd=shape(badpix)
                # sdf=shape(df)
            ...
            dfdsig = exp(-rhosq[lilrho[0]])*(rhosq[lilrho[0]]-1.)
            fpos = fsub[lilrho[0]]
            fposrow = where(fsub[lilrho[0]]-sky < 0.)
            fpos[fposrow[0]] = sky
